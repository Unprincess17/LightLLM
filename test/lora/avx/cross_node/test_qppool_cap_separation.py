...
def test_borrow_records_qp_wait_us():
    ...
    pool._qp_wait_us = deque(maxlen=100000)

    # Call the real borrow() — in preconnected mode it pops the transport
    pool_id, transport, ready_event = pool.borrow()
    assert len(pool._qp_wait_us) == 1
    assert pool._qp_wait_us[0] >= 0.0  # wait time is non-negative
