...
# For remote server startup: the worktree path doesn't exist on the remote
# node, but the relative path within the repo is the same.  Compute the path
# that works on the remote by using the main (non-worktree) repo root.
_GIT_TOP = subprocess.run(
    ["git", "rev-parse", "--show-toplevel"],
    capture_output=True, text=True, cwd=str(_HERE),
).stdout.strip()
_GIT_MAIN = subprocess.run(
    ["git", "rev-parse", "--path-format=absolute", "--git-common-dir"],
    capture_output=True, text=True, cwd=str(_HERE),
).stdout.strip()
if _GIT_TOP and _GIT_MAIN:
    _MAIN_REPO = str(Path(_GIT_MAIN).resolve().parent)
    _CROSS_NODE_REL = str(Path(_HERE).parent.relative_to(_GIT_TOP))
    _REMOTE_CROSS_NODE = f"{_MAIN_REPO}/{_CROSS_NODE_REL}"
else:
    _REMOTE_CROSS_NODE = str(_HERE.parent)
...
DEFAULT_NUM_QPS = 16
DEFAULT_BURST_US = 0         # continuous posting, no burst/gap
DEFAULT_GAP_US = 0           # rate limiting via post+poll throttling
...
class RDMATrafficGenerator:
    ...
    def start(self, bw_pct: int) -> None:
        ...
        # Scale QP count by bw_pct for graded HCA contention.
        # Even 1 QP saturates 200G, but more QPs = more CQ polling +
        # more SQ DMA + more PCIe pressure = more GLOO interference.
        # QP scaling is the only reliable way to create DIFFERENT
        # contention levels with RDMA (sleep-based rate limiting fails
        # because WRs complete asynchronously).
        qp_scale = max(1, int(self._num_qps * bw_pct / 100.0))
        ...
        self._start_remote_server(self._target_gbps, qp_scale)
    ...
    def _start_remote_server(self, target_gbps: int, num_qps: int) -> None:
        if not self._remote_ssh_host:
            return
        # Kill any stale server first (by port, by script name, and by import pattern)
        kill_cmds = (
            f"fuser -k {self._control_port}/tcp 2>/dev/null || true; "
            f"pkill -f rdma_qp_server_script 2>/dev/null || true; "
            f"pkill -f 'rdma_qp_pressure.*_run_server' 2>/dev/null || true"
        )
        subprocess.run(
            ["ssh", self._remote_ssh_host, kill_cmds],
            timeout=5, capture_output=True,
        )
        time.sleep(1)
        # Build server script and scp it to the remote, then launch it.
        # Using scp + ssh avoids heredoc/shell-quoting fragility.
        server_script = (
            f"import sys; sys.path.insert(0, '{_REMOTE_CROSS_NODE}'); "
            f"from rdma_qp_generator.rdma_qp_pressure import _run_server; "
            f"_run_server('{self._remote_ip}', {self._control_port}, "
            f"'{self._mlx_device}', {self._ib_port}, {num_qps}, "
            f"{self._qp_depth}, {self._msg_bytes}, {target_gbps})"
        )
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=".py", prefix="rdma_qp_server_", delete=False,
        ) as f:
        ...
        time.sleep(3)  # give the remote server time to bind
...
def _run_server(
    ...
    try:
        ...
        # Start burst loop (rate-limited to match client)
        ret = _lib.rdmaqp_start_burst_loop(ctx, 8000, 2000, target_gbps)
        if ret != 0:
