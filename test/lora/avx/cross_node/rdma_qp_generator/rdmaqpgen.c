    ...
    char        errbuf[256])
{
    ...
    qp_attr.sq_sig_all       = 1;  /* every WR generates a CQE */
...
static void* burst_loop_thread(void *arg) {
    ...
    while (!c->stop_requested) {
        /* --- Post: fill every QP's SQ completely --- */
        int total_posted = 0;
        ...
        /* --- Poll: drain all completions --- */
        int completed = 0;
        while (completed < total_posted && !c->stop_requested) {
