...
1. **Replace EP traffic generator** — Python IPoIB all-to-all confirms traffic and S2 degradation, but still lacks a monotonic curve; next use RDMA/QP-level (queue pair) all-to-all or real MoE traces with per-level IB counter summaries (one counter summary per EP-load level)
