...
def build_benchmark_command(run: LiveE2ERun, benchmark_script: str) -> str:
    ...
    if run.router_trace_path is not None:
        parts.append(f"--router_trace_path {run.router_trace_path}")
    ...
    if run.decode_target_tokens is not None:
        parts.append(f"--decode_target_tokens {run.decode_target_tokens}")
    if run.max_tokens is not None:
        parts.append(f"--max_tokens {run.max_tokens}")
    if run.warmup_decode_target_tokens is not None:
        parts.append(f"--warmup_decode_target_tokens {run.warmup_decode_target_tokens}")
